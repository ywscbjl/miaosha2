...
import miaoshaproject.mq.MqProducer;
...
import miaoshaproject.util.CodeUtil;
...
public class OrderController extends BaseController {
    ...
    public CommonReturnType generatetoken(@RequestParam(name="itemId")Integer itemId,
                                          ...
                                          @RequestParam(name="verifyCode")Integer verifyCode) throws BusinessException {
        //根据token获取用户信息
        String token = httpServletRequest.getParameterMap().get("token")[0];
        if(StringUtils.isEmpty(token)){
        ...
        if(!redisVerifyCode.equalsIgnoreCase(String.valueOf(verifyCode))){
            throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR,"请求非法");
        }
        //获取秒杀访问令牌
